__author__ = 'Gerasimos Matidis & Paul Groot'
import os
import re
import argparse
import configparser
                ...
                for i in study_list.index:   
                    ...
                    if invalid_date_profiles or invalid_email:
                        ...
                        if result is None:
                            ...
                            # check wheter a new profile coniguration is defined
        ...
        if m:
            ...
            if result is not None:
                ...
                for dirpath, dirname, filenames  in os.walk(new_folder_path):
                    for f in filenames:
                        if f.endswith(".dcm"):
                            ...
                            q2 = session.query(DICOMFilesTracking).filter(DICOMFilesTracking.sop_instance_uid == sop_uid)
                            result_2 = q2.one_or_none() 
                            if result_2 is None:
                                rec = DICOMFilesTracking(record_id=result.StudyExportRecord.record_id, sop_instance_uid=sop_uid, 
                                    study_instance_uid=st_uid)
                                session.add(rec)
                                session.commit()
                                q3 = session.query(DICOMFilesTracking).filter(
                                    DICOMFilesTracking.sop_instance_uid == sop_uid)

                                result3 = q3.one_or_none()
                                track_id_value = str(result3.track_id)
                ...
                received_num = session.query(func.count(DICOMFilesTracking.track_id)).filter(
                    DICOMFilesTracking.study_instance_uid == st_uid).scalar()
...
def ctp_check():
        ...
        for f_2 in filenames_2:
            if f_2.endswith('.dcm'):
                ...
                if hasattr(ds_2, "StudyID"):
                    ...
                    q_2 = session.query(DICOMFilesTracking).filter(DICOMFilesTracking.track_id == track_id_value_2)

                    result_2 = q_2.one_or_none()
                    if result_2:
        ...
        for f_3 in filenames_3:
            if f_3.endswith('.dcm'):
                ...
                if hasattr(ds_3, "StudyID"):
                    ...
                    # query to check if there is a record in the "DICOMFilesTracking" table, which also is in the 
                    q_3 = session.query(DICOMFilesTracking).filter(DICOMFilesTracking.track_id == track_id_value_3)

                    result_3 = q_3.one_or_none()
                    if result_3:
    ...
    # querry to select the records which are in the state "ExportedToCTP"    
    q_3 = session.query(StudyExportRecord).filter(
        or_(StudyExportRecord.state == StudyExportState.ExportedToCTP, StudyExportRecord.state == StudyExportState.Incomplete))
    
    for i in q_3:

        # initialize
        failures = i.number_of_failures
        msg = ''
        modif_time = i.modified 
        new_state = i.state
        ...
        anonymized_num = session.query(func.count(DICOMFilesTracking.track_id)).filter(
            and_(DICOMFilesTracking.record_id == i.record_id, DICOMFilesTracking.state == CTPExportState.Anonymized)).scalar()

        quarantine_num = session.query(func.count(DICOMFilesTracking.track_id)).filter(
            and_(DICOMFilesTracking.record_id == i.record_id, DICOMFilesTracking.state == CTPExportState.InQuarantine)).scalar()

        unclassified_num = session.query(func.count(DICOMFilesTracking.track_id)).filter(
            and_(DICOMFilesTracking.record_id == i.record_id, DICOMFilesTracking.state == CTPExportState.Unclassified)).scalar()
        ...
        if files_num == i.images_received:
            failures = 0
...
def extract_image_info():
    ...
    if all(elem in dirs for elem in label_dirs):
        ...
        '(see the "classification_directories" list in dicts.py). They must have been modified/removed.'
        logging.error(msg)  
        raise KeyError(msg)
            ...
            for f in filenames:
                if f.endswith('.dcm'):
                    ...
                    if result is None:
                        ...
                        # NOTE: the three labels have follow the hierarchy "unsafe-> burned-in-> safe" 
...
def classify_image_type(dicom_header):
    ...
    # if there is no match, forward the file to the "unclassified" folder
    if result is None:
        label = ""
...
if __name__ == "__main__":
    ...
    if os.path.isdir('/mnt/l-disk') is False:
        sys.exit()
    ...
    studymaker = subparsers.add_parser('study',
                        help='Looks into a pre-defined directory for "xls" files with a pre-specified structure format. '
                        'By extracting the relevant information from these files, it creates one or more new study_records '
                        '(if not already exist) and submits them to the DataBase.')
    ...
    studymaker_auto.add_argument('--source', default=config['STUDY']['SOURCE'], 
                        help='The source directory where the function looks for "xls" files. By extracting the relevant '
                        'information from these files, it creates one or more job records (related to some studies) and '
    ...
    studymaker_manual.add_argument('--no_cln_graphics', 
                        action='store_false',
                        help='Disables the Clean Graphics option. When this option is specified in addition to an Application '
                            'Level Confidentiality Profile, any information encoded in graphics, text annotations or overlays '
                            'corresponding to the Attribute information specified to be removed by the Profile and any other '
    ...
    studymaker_manual.add_argument('--no_cln_structured_content', 
                        ...
                        help='Disables the Clean Structured Content option. When this Option is specified in addition to an Application '
                            'Level Confidentiality Profile, any information encoded in SR Content Items or Acquisition Context or '
                            'Specimen Preparation Sequence Items corresponding to the Attribute information specified to be removed by '
                            'the Profile and any other options specified shall also be removed.')
    studymaker_manual.add_argument('--no_cln_descriptors', 
                        ...
                        help='Disables the Clean Descriptors option. When this Option is specified in addition to an Application Level '
                            'Confidentiality Profile, any information that is embedded in text or string Attributes corresponding to '
                            'the Attribute information specified to be removed by the Profile and any other Options specified shall '
                            'also be removed.')
    studymaker_manual.add_argument('--rtn_longitudinal_full_dates', 
                        action='store_true',
                        help='Enables the Retain Longitudinal Temporal Information With Full Dates option. When this option is specified '
                            'in addition to an Application Level Confidentiality Profile, any dates and times present in the Attributes '
                            'shall be retained. The Attribute Longitudinal Temporal Information Modified (0028,0303) shall be added to '
    ...
    studymaker_manual.add_argument('--rtn_longitudinal_modified_dates', 
                        ...
                        help='Enables the Retain Longitudinal Temporal Information With Modified Dates option. When this option is specified '
                            'in addition to an Application Level Confidentiality Profile, any dates and times present in the Attributes '
                            'shall be modified. The modification of the dates and times shall be performed in a manner that: \n' 
                            'aggregates or transforms dates so as to reduce the possibility of matching for re-identification \n'
                            'preserves the gross longitudinal temporal relationships between images obtained on different dates \n'
                            'to the extent necessary for the application \n preserves the fine temporal relationships between images and '
                            'real-world events to the extent necessary for analysis of the images for the application \n'
    ...
    jobmaker = subparsers.add_parser('job',
                        help='Looks into a pre-defined directory for "xls" files with a pre-specified structure format. '
                        'If it finds new DICOM studies that related to an excisting study acrony, then it creates a new job and '
    ...
    finder = subparsers.add_parser('find', 
                        help='Applies the C-FIND service operation by which relevant patient ' 
                        'information is queried by the user AE and provided by the PACS.')
    ...
    finder.add_argument('--timelimit', default=config['FIND']['RETRY_INTERVAL'],
                        help='This argument indicates a time interval, which is used from the function in order to select '
                        'the records in the DB which have been modified before this.')
    finder.add_argument('--failures_limit', default=config['FIND']['FAILURES_LIMIT'],
                        help='This argument indicates the maximum number of the failures a record in the DB is allowed to have, '
                        'before it is set to the "Failed" state.')
    
    mover = subparsers.add_parser('move', 
                        help='Applies the C-MOVE service operation which allows the user AE ' 
                            'to instruct PACS to transfer stored SOP Instances to another AE.')
    ...
    mover.add_argument('--timelimit', default=config['MOVE']['RETRY_INTERVAL'],
                        help='This argument indicates a time interval, which is used from the function in order to select '
                        'the records in the DB which have been modified before this.')
    mover.add_argument('--failures_limit', default=config['MOVE']['FAILURES_LIMIT'],
                        help='This argument indicates the maximum number of the failures a record in the DB is allowed to have, '
                        'before it is set to the "Failed" state.')
    
    preparer = subparsers.add_parser('prepare',
                        help='Prepares the DICOM files for the CTP pipeline, by modifying some specific tags. '
                        'At the same time 1) it enumerates them and updates the "StudyExportRecord" table and '
                        '2) it adds a record to the "DICOMFilesTracking" table with their SOP Instance UID, '
                        'as well as the respective Study Instance UID.')
    preparer.set_defaults(func=prepare_session)
    preparer.add_argument('--source', default=config['PREPARE']['SOURCE'], 
                        help='The source directory where the DICOM files are retrieved. Usually it is '
                        'the directory where the files are stored after the C-MOVE service operation.')
    ...
    preparer.add_argument('--timelimit', default=config['PREPARE']['RETRY_INTERVAL'],
                        help='This argument indicates a time interval, which is used from the function in order to select '
                        'the records in the DB which have been modified before this.')
    preparer.add_argument('--failures_limit', default=config['PREPARE']['FAILURES_LIMIT'],
                        help='This argument indicates the maximum number of the failures a record in the DB is allowed to have, '
                        'before it is set to the "Failed" state.')

    ctpchecker = subparsers.add_parser('ctpcheck',
                        help="It searches the CTP's output directories (anonymized and quarantine) for dcm files. For every dcm file "
                        "checks if exists in the DB. If so, it updates the StudyExportRecord table by increasing the number of the images "
                        'that were anonynized or in quarantine by CTP and forwards the file to the "counted" directory. Additionally, '
                        "it updates the DICOMFilesTracking table with the state of every file. The function also forwards all the dcm "
                        'files that not exist in the DB in the "counted" directory.')
    ...
    ctpchecker.add_argument('--timelimit', default=config['CTPCHECK']['RETRY_INTERVAL'],
                        help='This argument indicates a time interval, which is used from the function in order to select '
                        'the records in the DB which have been modified before this.')
    ctpchecker.add_argument('--failures_limit', default=config['CTPCHECK']['FAILURES_LIMIT'],
                        help='This argument indicates the maximum number of the failures a record in the DB is allowed to have, '
                        'before it is set to the "Failed" state.')

    extractor = subparsers.add_parser('extract',
                        help='Looks into three pre-defined directories for new DICOM files and then extracts relevant '
                            'information from the DICOM header. Then, makes a query to the database to check if this '
                            'information matches to any of the existing records. If not, it creates a new record and '
                            'additionally adds a classification label to the new record (according to which of the three '
                            'directories the file comes from) so it can be used in the regular workflow to automatically '
                            'classify similar images that have matching DICOM attributes. The files that correspond to '
                            'new records are being archived into a specific folder.')
